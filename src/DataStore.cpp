/** @file DataStore.cpp
 *  @brief Filesystem persistence implementation.
 */
...
#endif
#if defined(NRF52_PLATFORM) || defined(STM32_PLATFORM)
    _ContactsChannelsTotalBlocks =
        _getContactsChannelsFS()->_getFS()->cfg->block_count;
    checkAdvBlobFile();
#if defined(EXTRAFS) || defined(QSPIFLASH)
...
/**
 * @brief Counts one traversed LittleFS block.
 * @param p Block-count accumulator.
 * @param block Traversed block number.
 * @return LittleFS traversal status.
 */
int _countLfsBlock(void *p, lfs_block_t block) {
    if (block > _ContactsChannelsTotalBlocks) {
        MESH_DEBUG_PRINTLN(
            "ERROR: Block %d exceeds filesystem bounds - CORRUPTION DETECTED!",
            block);
        return LFS_ERR_CORRUPT; // return error to abort lfs_traverse()
                                // gracefully
    }
...
/**
 * @brief Counts blocks used by a LittleFS filesystem.
 * @param fs Filesystem to inspect.
 * @return Number of used blocks, or zero on traversal failure.
 */
...
bool DataStore::formatFileSystem() {
    ...
    esp_err_t nvs_err =
        nvs_flash_erase(); // no need to reinit, will be done by reboot
    return fs_success && (nvs_err == ESP_OK);
#elif defined(NATIVE_BUILD)
    // native unit-test build: no real hardware filesystem/NVS to format
    return _fs->format();
#else
...
/**
 * @brief Loads node preferences from the legacy binary format.
 * @param filename Legacy preference file path.
 * @param _prefs Destination preferences.
 * @return None.
 */
void DataStore::loadPrefsInt(const char *filename, NodePrefs &_prefs) {
    ...
    if (file) {
        ...
        file.read((uint8_t *)&_prefs._client_repeat,
                  sizeof(_prefs._client_repeat)); // 62
        file.read((uint8_t *)&_prefs.manual_add_contacts,
                  sizeof(_prefs.manual_add_contacts));       // 63
        file.read((uint8_t *)&_prefs.bw, sizeof(_prefs.bw)); // 64
        file.read((uint8_t *)&_prefs.tx_power_dbm,
                  sizeof(_prefs.tx_power_dbm)); // 68
        file.read((uint8_t *)&_prefs.telemetry_mode_base,
                  sizeof(_prefs.telemetry_mode_base)); // 69
        file.read((uint8_t *)&_prefs.telemetry_mode_loc,
                  sizeof(_prefs.telemetry_mode_loc)); // 70
        file.read((uint8_t *)&_prefs.telemetry_mode_env,
                  sizeof(_prefs.telemetry_mode_env)); // 71
        file.read((uint8_t *)&_prefs.rx_delay_base,
                  sizeof(_prefs.rx_delay_base)); // 72
        file.read((uint8_t *)&_prefs.advert_loc_policy,
                  sizeof(_prefs.advert_loc_policy)); // 76
        file.read((uint8_t *)&_prefs.multi_acks,
                  sizeof(_prefs.multi_acks)); // 77
        file.read((uint8_t *)&_prefs.path_hash_mode,
                  sizeof(_prefs.path_hash_mode));                      // 78
        file.read(pad, 1);                                             // 79
        file.read((uint8_t *)&_prefs.ble_pin, sizeof(_prefs.ble_pin)); // 80
        file.read((uint8_t *)&_prefs.buzzer_quiet,
                  sizeof(_prefs.buzzer_quiet)); // 84
        file.read((uint8_t *)&_prefs.gps_enabled,
                  sizeof(_prefs.gps_enabled)); // 85
        file.read((uint8_t *)&_prefs.gps_interval,
                  sizeof(_prefs.gps_interval)); // 86
        file.read((uint8_t *)&_prefs.autoadd_config,
                  sizeof(_prefs.autoadd_config)); // 87
        file.read((uint8_t *)&_prefs.autoadd_max_hops,
                  sizeof(_prefs.autoadd_max_hops)); // 88
        file.read((uint8_t *)&_prefs.rx_boosted_gain,
                  sizeof(_prefs.rx_boosted_gain)); // 89
        file.read((uint8_t *)_prefs.default_scope_name,
                  sizeof(_prefs.default_scope_name)); // 90
        file.read((uint8_t *)_prefs.default_scope_key,
                  sizeof(_prefs.default_scope_key)); // 121
        // migrate old fields
 ...
 * @param _prefs Preferences to save.
 * @return true when the write succeeds; otherwise false.
 */
    ...
    if (file) {
        ...
        while (!full) {
            ...
            success = success && (file.read((uint8_t *)&c.sync_since, 4) ==
                                  4); // was 'reserved'
            success =
                success && (file.read((uint8_t *)&c.out_path_len, 1) == 1);
            success = success &&
...
void DataStore::saveContacts(DataStoreHost *host,
    ...
    if (file) {
        ...
        while (host->getContactForSave(idx, c)) {
            ...
            success =
                success && (file.write((uint8_t *)&c.out_path_len, 1) == 1);
            success = success &&
...
void DataStore::loadChannels(DataStoreHost *host) {
    ...
    if (file) {
        ...
        while (!full) {
            ...
            success =
                success && (file.read((uint8_t *)ch.channel.secret, 32) == 32);
            if (!success)
                break; // EOF
...
void DataStore::saveChannels(DataStoreHost *host) {
    ...
    if (file) {
        ...
        while (host->getChannelForSave(channel_idx, ch)) {
            ...
            success =
                success && (file.write((uint8_t *)ch.channel.secret, 32) == 32);
            if (!success)
                break; // write failed
            channel_idx++;
        }
...
/**
 * @brief Creates the fixed advertisement blob file when absent.
 * @param None.
 * @return None.
 */
void DataStore::checkAdvBlobFile() {
    if (!_getContactsChannelsFS()->exists("/adv_blobs")) {
        ...
        if (file) {
            ...
            for (int i = 0; i < MAX_BLOBRECS;
                 i++) { // pre-allocate to fixed size
...
/**
 * @brief Migrates supported data to the secondary filesystem.
 * @param None.
 * @return None.
 */
void DataStore::migrateToSecondaryFS() {
    // migrate old adv_blobs, contacts3 and channels2 files to secondary FS if
    ...
    if (!_fsExtra->exists("/adv_blobs")) {
        if (_fs->exists("/adv_blobs")) {
            ...
            if (oldAdvBlobs && newAdvBlobs) {
                ...
                while (count < 20 &&
                       oldAdvBlobs.read((uint8_t *)&rec, sizeof(rec)) ==
                           sizeof(rec)) {
    ...
    // cleanup nodes which have been testing the extra fs, copy _main.id and
    // new_prefs back to primary
...
uint8_t DataStore::getBlobByKey(const uint8_t key[], int key_len,
    ...
    if (file) {
        ...
        while (file.read((uint8_t *)&tmp, sizeof(tmp)) == sizeof(tmp)) {
            if (memcmp(key, tmp.key, sizeof(tmp.key)) ==
                0) { // only match by 7 byte prefix
                len = tmp.len;
...
bool DataStore::putBlobByKey(const uint8_t key[], int key_len,
    ...
    if (file) {
        ...
        memcpy(tmp.key, key,
               sizeof(tmp.key)); // just record 7 byte prefix of key
...
/**
 * @brief Deletes an advertisement blob on filesystem-backed targets.
 * @param key Blob key.
...
/**
 * @brief Builds the filesystem path for a blob key.
 * @param key Blob key.
 * @param key_len Key length.
 ...
 * @return None.
 */
inline void makeBlobPath(const uint8_t key[], int key_len, char *path,
                         size_t path_size) {
    char fname[18];
    if (key_len > 8)
...
/**
 * @brief Reads a filesystem blob by key.
 * @param key Blob key.
 * @param key_len Key length.
...
uint8_t DataStore::getBlobByKey(const uint8_t key[], int key_len,
    ...
    if (_fs->exists(path)) {
        ...
        if (f) {
            int len = f.read(
                dest_buf, 255); // currently MAX 255 byte blob len supported!!
            f.close();
            return len;
        }
    }
...
/**
 * @brief Writes a filesystem blob by key.
 * @param key Blob key.
 * @param key_len Key length.
...
/**
 * @brief Deletes a filesystem blob by key.
 * @param key Blob key.
 * @param key_len Key length.
