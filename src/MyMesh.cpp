/** @file MyMesh.cpp
 *  @brief MeshCore application and protocol implementation.
 */
...
#define RESP_CODE_CONTACT_MSG_RECV                                             \
    7 // a reply to CMD_SYNC_NEXT_MESSAGE (ver < 3)
#define RESP_CODE_CHANNEL_MSG_RECV                                             \
    8                         // a reply to CMD_SYNC_NEXT_MESSAGE (ver < 3)
...
#define RESP_CODE_CONTACT_MSG_RECV_V3                                          \
    16 // a reply to CMD_SYNC_NEXT_MESSAGE (ver >= 3)
#define RESP_CODE_CHANNEL_MSG_RECV_V3                                          \
    17                            // a reply to CMD_SYNC_NEXT_MESSAGE (ver >= 3)
...
#define PUSH_CODE_CONTACT_DELETED                                              \
    0x8F // used to notify client app of deleted contact when overwriting oldest
#define PUSH_CODE_CONTACTS_FULL                                                \
    0x90 // used to notify client app that contacts storage is full
...
// Bit 0: If set, overwrite oldest non-favourite contact when contacts file is
// full Bits 1-4: these indicate which contact types to auto-add when
// manual_contact_mode = 0x01
#define AUTO_ADD_OVERWRITE_OLDEST                                              \
    (1 << 0) // 0x01 - overwrite oldest non-favourite when full
#define AUTO_ADD_CHAT                                                          \
    (1 << 1) // 0x02 - auto-add Chat (Companion) (ADV_TYPE_CHAT)
#define AUTO_ADD_REPEATER                                                      \
    (1 << 2) // 0x04 - auto-add Repeater (ADV_TYPE_REPEATER)
#define AUTO_ADD_ROOM_SERVER                                                   \
...
/**
 * @brief Writes a serialized contact response.
...
/**
 * @brief Decodes contact data from an application frame.
 ...
 * @return None.
 */
void MyMesh::updateContactFromFrame(ContactInfo &contact, uint32_t &last_mod,
...
/**
 * @brief Reports whether this queued frame is a channel message.
 * @param None.
 * @return true for a channel message; otherwise false.
 */
bool MyMesh::Frame::isChannelMsg() const {
    return buf[0] == RESP_CODE_CHANNEL_MSG_RECV ||
    ...
    if (offline_queue_len >= OFFLINE_QUEUE_SIZE) {
        ...
        while (pos < offline_queue_len) {
            if (offline_queue[pos].isChannelMsg()) {
                for (int i = pos; i < offline_queue_len - 1;
                     i++) { // delete oldest channel msg from queue
                    offline_queue[i] = offline_queue[i + 1];
                }
                MESH_DEBUG_PRINTLN(
                    "INFO: removed oldest channel message from queue.");
 ...
 */
float MyMesh::getAirtimeBudgetFactor() const { return _prefs.airtime_factor; }
...
/**
 * @brief Reports whether channel activity detection is enabled.
 * @param None.
 * @return true when enabled; otherwise false.
 */
bool MyMesh::getCADEnabled() const {
    return false; // hardware CAD before TX (disabled by default, until
                  // configurable)
}

/**
 * @brief Calculates receive delay from score and airtime.
...
uint32_t MyMesh::getRetransmitDelay(const mesh::Packet *packet) {
    uint32_t t = (_radio->getEstAirtimeFor(packet->getPathByteLen() +
...
/**
 * @brief Returns the configured extra acknowledgement count.
 * @param None.
 * @return Extra acknowledgement count.
 */
uint8_t MyMesh::getExtraAckTransmitCount() const { return _prefs.multi_acks; }

/**
 * @brief Forwards raw received radio data to the serial interface.
 ...
 * @param contact_type Contact advertisement type.
 ...
 */
uint8_t MyMesh::getAutoAddMaxHops() const { return _prefs.autoadd_max_hops; }
...
void MyMesh::onDiscoveredContact(ContactInfo &contact, bool is_new,
    ...
    if (path && mesh::Packet::isValidPathLen(path_len)) { // check path is valid
        ...
        for (int i = 0; i < ADVERT_PATH_TABLE_SIZE;
            ...
            if (memcmp(advert_paths[i].pubkey_prefix, contact.id.pub_key,
    ...
    if (!is_new)
        dirty_contacts_expiry = futureMillis(
            LAZY_CONTACTS_WRITE_DELAY); // only schedule lazy write for contacts
                                        // that are in contacts[]
}

/**
 * @brief Compares advertisement paths by receive time.
 * @param a First advertisement path.
 * @param b Second advertisement path.
 * @return Negative, zero, or positive comparison result.
 */
static int sort_by_recent(const void *a, const void *b) {
    return ((AdvertPath *)b)->recv_timestamp -
           ((AdvertPath *)a)->recv_timestamp;
}
...
int MyMesh::getRecentlyHeard(AdvertPath dest[], int max_num) {
    ...
    qsort(advert_paths, ADVERT_PATH_TABLE_SIZE, sizeof(advert_paths[0]),
          sort_by_recent);
...
ContactInfo *MyMesh::processAck(const uint8_t *data) {
    ...
    for (int i = 0; i < EXPECTED_ACK_TABLE_SIZE; i++) {
        if (memcmp(data, &expected_ack_table[i].ack, 4) ==
            0) { // got an ACK from recipient
            out_frame[0] = PUSH_CODE_SEND_CONFIRMED;
            memcpy(&out_frame[1], data, 4);
            uint32_t trip_time =
                _ms->getMillis() - expected_ack_table[i].msg_sent;
            memcpy(&out_frame[5], &trip_time, 4);
            _serial->writeFrame(out_frame, 9);
            // NOTE: the same ACK can be received multiple times!
            expected_ack_table[i].ack =
                0; // clear expected hash, now that we have received ACK
...
void MyMesh::queueMessage(const ContactInfo &from, uint8_t txt_type,
    ...
    bool should_display =
        txt_type == TXT_TYPE_PLAIN || txt_type == TXT_TYPE_SIGNED_PLAIN;
...
/**
 * @brief Filters a received flood packet.
 * @param packet Incoming packet.
 * @return true when accepted; otherwise false.
 */
bool MyMesh::filterRecvFloodPacket(mesh::Packet *packet) {
    // REVISIT: try to determine which Region (from transport_codes[1]) that
    // Sender is indicating for replies/responses
    //    if unknown, fallback to finding Region from transport_codes[0], the
    //    'scope' used by Sender
    return false;
}
...
void MyMesh::sendFloodScoped(const TransportKey &scope, mesh::Packet *pkt,
    ...
    if (scope.isNull()) {
        ...
        codes[1] =
            0; // REVISIT: set to 'home' Region, for sender/return region?
...
void MyMesh::sendFloodScoped(const ContactInfo &recipient, mesh::Packet *pkt,
                             uint32_t delay_millis) {
    // TODO: dynamic send_scope, depending on recipient and current 'home'
    // Region
    if (send_unscoped) {
        sendFlood(pkt, delay_millis,
                  _prefs.path_hash_mode +
                      1); // app has explicitly requested un-scoped
    } else {
        TransportKey default_scope;
        memcpy(&default_scope.key, _prefs.default_scope_key,
 ...
 * @return None.
 */
void MyMesh::sendFloodScoped(const mesh::GroupChannel &channel,
                             mesh::Packet *pkt, uint32_t delay_millis) {
 ...
 * @return None.
 */
void MyMesh::onCommandDataRecv(const ContactInfo &from, mesh::Packet *pkt,
                               uint32_t sender_timestamp, const char *text) {
    markConnectionActive(
        from); // in case this is from a server, and we have a connection
    queueMessage(from, TXT_TYPE_CLI_DATA, pkt, sender_timestamp, nullptr, 0,
                 text);
}
 ...
 * @return None.
 */
void MyMesh::onSignedMessageRecv(const ContactInfo &from, mesh::Packet *pkt,
 ...
 * @return None.
 */
void MyMesh::onChannelMessageRecv(const mesh::GroupChannel &channel,
...
void MyMesh::onChannelDataRecv(const mesh::GroupChannel &channel,
    ...
    if (data_len > MAX_CHANNEL_DATA_LENGTH) {
        MESH_DEBUG_PRINTLN(
            "onChannelDataRecv: dropping payload_len=%d exceeds frame limit=%d",
...
uint8_t MyMesh::onContactRequest(const ContactInfo &contact,
    ...
    if (data[0] == REQ_TYPE_GET_TELEMETRY_DATA) {
        ...
        uint8_t cp = contact.flags >>
                     1; // LSB used as 'favourite' bit (so only use upper bits)
        ...
        uint8_t perm_mask =
            ~(data[1]); // NEW: first reserved byte (of 4), is now inverse mask
                        // to apply to permissions
        permissions &= perm_mask;
        if (permissions &
            TELEM_PERM_BASE) { // only respond if base permission bit is set
            telemetry.reset();
            ...
            if (!isnan(temperature)) { // Supported boards with built-in
                                       // temperature sensor. ESP32-C3 may
                                       // return NAN
            ...
            memcpy(reply, &sender_timestamp,
                   4); // reflect sender_timestamp back in response packet (kind
                       // of like a 'tag')
 ...
 * @return None.
 */
void MyMesh::onContactResponse(const ContactInfo &contact, const uint8_t *data,
                               uint8_t len) {
    uint32_t tag;
    memcpy(&tag, data, 4);
    if (pending_login && memcmp(&pending_login, contact.id.pub_key, 4) ==
        ...
        if (memcmp(&data[4], "OK", 2) ==
            0) { // legacy Repeater login OK response
        ...
        _serial->writeFrame(out_frame, i);
               ...
               tag == pending_telemetry) { // check for matching response tag
        pending_telemetry = 0;
        int i = 0;
        ...
        _serial->writeFrame(out_frame, i);
               ...
               tag == pending_req) { // check for matching response tag
        pending_req = 0;
        int i = 0;
        out_frame[i++] = PUSH_CODE_BINARY_RESPONSE;
        out_frame[i++] = 0; // reserved
        memcpy(&out_frame[i], &tag,
               4); // app needs to match this to RESP_CODE_SENT.tag
        i += 4;
        memcpy(&out_frame[i], &data[4], len - 4);
        i += (len - 4);
        _serial->writeFrame(out_frame, i);
 ...
 * @param contact Contact that supplied the path.
 ...
 * @return true when accepted; otherwise false.
 */
bool MyMesh::onContactPathRecv(ContactInfo &contact, uint8_t *in_path,
    ...
    if (extra_type == PAYLOAD_TYPE_RESPONSE && extra_len > 4) {
        ...
        if (tag == pending_discovery) { // check for matching response tag)
            ...
            if (!mesh::Packet::isValidPathLen(in_path_len) ||
                ...
                MESH_DEBUG_PRINTLN(
                    "onContactPathRecv, invalid path sizes: %d, %d",
                    in_path_len, out_path_len);
            } else {
                int i = 0;
                ...
                i += mesh::Packet::writePath(&out_frame[i], out_path,
                                             out_path_len);
                out_frame[i++] = in_path_len;
                i += mesh::Packet::writePath(&out_frame[i], in_path,
                                             in_path_len);
    ...
    return BaseChatMesh::onContactPathRecv(contact, in_path, in_path_len,
                                           out_path, out_path_len, extra_type,
                                           extra, extra_len);
}
...
void MyMesh::onControlDataRecv(mesh::Packet *packet) {
    if (packet->payload_len + 4 > sizeof(out_frame)) {
        MESH_DEBUG_PRINTLN("onControlDataRecv(), payload_len too long: %d",
                           packet->payload_len);
        return;
    }
    int i = 0;
    ...
    if (_serial->isConnected()) {
        ...
        MESH_DEBUG_PRINTLN(
            "onControlDataRecv(), data received while app offline");
    }
}
...
void MyMesh::onRawDataRecv(mesh::Packet *packet) {
    if (packet->payload_len + 4 > sizeof(out_frame)) {
        MESH_DEBUG_PRINTLN("onRawDataRecv(), payload_len too long: %d",
                           packet->payload_len);
        return;
    }
    int i = 0;
...
void MyMesh::onTraceRecv(mesh::Packet *packet, uint32_t tag, uint32_t auth_code,
    ...
    if (12 + path_len + (path_len >> path_sz) + 1 > sizeof(out_frame)) {
        MESH_DEBUG_PRINTLN("onTraceRecv(), path_len is too long: %d",
                           (uint32_t)path_len);
        return;
    }
    int i = 0;
    ...
    i += path_len >> path_sz;
    out_frame[i++] =
        (int8_t)(packet->getSNR() * 4); // extra/final SNR (to this node)
    if (_serial->isConnected()) {
        _serial->writeFrame(out_frame, i);
    } else {
 ...
 * @return Timeout in milliseconds.
 */
uint32_t MyMesh::calcDirectTimeoutMillisFor(uint32_t pkt_airtime_millis,
 ...
 * @return None.
 */
MyMesh::MyMesh(mesh::Radio &radio, mesh::MillisecondClock &millis,
               mesh::PacketManager &packet_manager, mesh::RNG &rng,
               mesh::RTCClock &rtc, SimpleMeshTables &tables, DataStore &store,
               AbstractUITask *ui)
    : BaseChatMesh(radio, millis, rng, rtc, packet_manager, tables),
      _serial(nullptr), telemetry(MAX_PACKET_PAYLOAD - 4), _store(&store),
      _ui(ui), _iter(0) {
    _iter_started = false;
    _cli_rescue = false;
    offline_queue_len = 0;
...
void MyMesh::begin(bool has_display) {
    ...
    if (!_store->loadMainIdentity(self_id)) {
        ...
        int count = 0;
        while (count < 10 &&
               (self_id.pub_key[0] == 0x00 ||
                self_id.pub_key[0] == 0xFF)) { // reserved id hashes
...
// if name is provided as a build flag, use that as default node name instead
    ...
    _prefs.gps_enabled =
        constrain(_prefs.gps_enabled, 0, 1); // Ensure boolean 0 or 1
    _prefs.gps_interval =
        constrain(_prefs.gps_interval, 0, 86400); // Max 24 hours
    ...
    if (_prefs.ble_pin == 0) {
        ...
        if (has_display && BLE_PIN_CODE == 123456) {
            StdRNG rng;
            _active_ble_pin =
                rng.nextInt(100000, 999999); // random pin each session
        } else {
    ...
    MESH_DEBUG_PRINTLN("RX Boosted Gain Mode: %s",
                       radio_driver.getRxBoostedGainMode() ? "Enabled"
                                                           : "Disabled");
}
 ...
 */
NodePrefs *MyMesh::getNodePrefs() { return &_prefs; }
...
bool MyMesh::isValidClientRepeatFreq(uint32_t f) const {
    for (int i = 0;
         i < sizeof(repeat_freq_ranges) / sizeof(repeat_freq_ranges[0]); i++) {
...
void MyMesh::handleCmdFrame(size_t len) {
    if (cmd_frame[0] == CMD_DEVICE_QUERY &&
        ...
        out_frame[i++] = (_prefs.telemetry_mode_env << 4) |
                         (_prefs.telemetry_mode_loc << 2) |
        ...
        if (recipient &&
            ...
            if (txt_type == TXT_TYPE_CLI_DATA) {
                msg_timestamp =
                    getRTCClock()
                        ->getCurrentTimeUnique(); // Use node's RTC instead of
                                                  // app timestamp to avoid
                                                  // tripping replay protection
                result = sendCommandData(*recipient, msg_timestamp, attempt,
                                         text, est_timeout);
            ...
            if (result == MSG_SEND_FAILED) {
                ...
                if (expected_ack) {
                    expected_ack_table[next_ack_idx].msg_sent =
                        _ms->getMillis(); // add to circular table
            ...
            writeErrFrame(
                recipient == nullptr
                    ? ERR_CODE_NOT_FOUND
                    : ERR_CODE_UNSUPPORTED_CMD); // unknown recipient, or
                                                 // unsupported TXT_TYPE_*
        }
    } else if (cmd_frame[0] ==
               CMD_SEND_CHANNEL_TXT_MSG) { // send GroupChannel text msg
        int i = 1;
        ...
        if (txt_type != TXT_TYPE_PLAIN) {
            ...
            if (success && sendGroupMessage(msg_timestamp, channel.channel,
                                            _prefs.node_name, text, len - i)) {
                writeOKFrame();
            } else {
               ...
               CMD_SEND_CHANNEL_DATA) { // send GroupChannel datagram
        if (len < 4) {
        ...
        if (!mesh::Packet::isValidPathLen(path_len) &&
            ...
            MESH_DEBUG_PRINTLN("CMD_SEND_CHANNEL_DATA invalid path size: %d",
                               path_len);
            writeErrFrame(ERR_CODE_ILLEGAL_ARG);
            return;
        }
        ...
        uint16_t data_type =
            ((uint16_t)cmd_frame[i]) | (((uint16_t)cmd_frame[i + 1]) << 8);
        i += 2;
        ...
        if (!getChannel(channel_idx, channel)) {
            ...
            MESH_DEBUG_PRINTLN(
                "CMD_SEND_CHANNEL_DATA payload too long: %d > %d", payload_len,
                MAX_CHANNEL_DATA_LENGTH);
            writeErrFrame(ERR_CODE_ILLEGAL_ARG);
        } else if (sendGroupData(channel.channel, path, path_len, data_type,
                                 payload, payload_len)) {
            writeOKFrame();
        } else {
            writeErrFrame(ERR_CODE_TABLE_FULL);
        ...
        if (_prefs.advert_loc_policy == ADVERT_LOC_NONE) {
            ...
            pkt = createSelfAdvert(_prefs.node_name, sensors.node_lat,
                                   sensors.node_lon);
        }
        if (pkt) {
            if (len >= 2 &&
        ...
        if (recipient) {
            ...
            // recipient->lastmod = ??   shouldn't be needed, app already has
            // this version of contact
            dirty_contacts_expiry = futureMillis(LAZY_CONTACTS_WRITE_DELAY);
            writeOKFrame();
        } else {
        ...
        ContactInfo *recipient = lookupContactByPubKey(pub_key, PUB_KEY_SIZE);
        uint32_t last_mod =
            getRTCClock()->getCurrentTime(); // fallback value if not present in
                                             // cmd_frame
        if (recipient) {
        ...
        if (len < 1 + PUB_KEY_SIZE) {
            ...
            if (pkt) {
                pkt->header |=
                    ROUTE_TYPE_FLOOD; // would normally be sent in this mode
        ...
        if (repeat && !isValidClientRepeatFreq(freq)) {
            writeErrFrame(ERR_CODE_ILLEGAL_ARG);
        } else if (freq >= 150000 && freq <= 2500000 && sf >= 5 && sf <= 12 &&
            ...
            savePrefs();
            radio_driver.setParams(_prefs.freq, _prefs.bw, _prefs.sf,
                                   _prefs.cr);
            MESH_DEBUG_PRINTLN(
            ...
            MESH_DEBUG_PRINTLN(
                "Error: CMD_SET_RADIO_PARAMS: f=%d, bw=%d, sf=%d, cr=%d", freq,
                bw, (uint32_t)sf, (uint32_t)cr);
            writeErrFrame(ERR_CODE_ILLEGAL_ARG);
        }
        ...
        savePrefs();
        writeOKFrame();
    } else if (cmd_frame[0] == CMD_SET_PATH_HASH_MODE && cmd_frame[1] == 0 &&
               len >= 3) {
        if (cmd_frame[2] >= 3) {
            writeErrFrame(ERR_CODE_ILLEGAL_ARG);
        } else {
        ...
        if (dirty_contacts_expiry) { // is there are pending dirty contacts
                                     // write needed?
            saveContacts();
        }
        board.reboot();
        ...
        if (path_len >= 0 &&
            i + path_len + 4 <= len) { // minimum 4 byte payload
            ...
            writeErrFrame(
                ERR_CODE_UNSUPPORTED_CMD); // flood, not supported (yet)
        }
        ...
        ContactInfo anon{};
        if (recipient ==
            nullptr) { // FIRMWARE_VER_CODE 13+,  allow non-contact requests
        ...
        if (recipient) {
            ...
            int result = sendAnonReq(*recipient, data, len - (1 + PUB_KEY_SIZE),
                                     tag, est_timeout);
            if (result == MSG_SEND_FAILED) {
                writeErrFrame(ERR_CODE_TABLE_FULL);
            } else {
        ...
        if (recipient) {
            ...
            if (result == MSG_SEND_FAILED) {
                ...
                clearPendingReqs();
                // FUTURE:  pending_status = tag;  // match this in
                // onContactResponse()
                memcpy(&pending_status, recipient->id.pub_key,
                       4); // legacy matching scheme
                out_frame[0] = RESP_CODE_SENT;
                out_frame[1] = (result == MSG_SEND_SENT_FLOOD) ? 1 : 0;
                memcpy(&out_frame[2], &tag, 4);
                memcpy(&out_frame[6], &est_timeout, 4);
    ...
    } else if (cmd_frame[0] == CMD_SEND_PATH_DISCOVERY_REQ &&
               cmd_frame[1] == 0 && len >= 2 + PUB_KEY_SIZE) {
        ...
        if (recipient) {
            ...
            req_data[1] =
                ~(TELEM_PERM_BASE);     // NEW: inverse permissions mask (ie. we
                                        // only want BASE telemetry)
            memset(&req_data[2], 0, 3); // reserved
            getRNG()->random(&req_data[5],
                             4); // random blob to help make packet-hash unique
            auto save =
                recipient
                    ->out_path_len; // temporarily force sendRequest() to flood
            recipient->out_path_len = OUT_PATH_UNKNOWN;
            int result = sendRequest(*recipient, req_data, sizeof(req_data),
                                     tag, est_timeout);
            ...
            writeErrFrame(ERR_CODE_NOT_FOUND); // contact not found
               ...
               len >= 4 + PUB_KEY_SIZE) { // can deprecate, in favour of
                                          // CMD_SEND_BINARY_REQ
        ...
        if (recipient) {
            ...
            uint32_t tag, est_timeout;
            int result =
                sendRequest(*recipient, req_data, len - (1 + PUB_KEY_SIZE), tag,
                            est_timeout);
            if (result == MSG_SEND_FAILED) {
                writeErrFrame(ERR_CODE_TABLE_FULL);
            } else {
        ...
        memcpy(channel.channel.secret, &cmd_frame[2 + 32], 32);
        ...
        memcpy(channel.channel.secret, &cmd_frame[2 + 32],
               16); // NOTE: only 128-bit supported
        if (setChannel(channel_idx, channel)) {
            saveChannels();
            writeOKFrame();
        } else {
        ...
        if (sign_data == nullptr ||
            sign_data_len + (len - 1) > MAX_SIGN_DATA_LEN) {
            writeErrFrame(sign_data == nullptr
                              ? ERR_CODE_BAD_STATE
                              : ERR_CODE_TABLE_FULL); // error: too long
        } else {
        ...
        if (sign_data) {
            ...
            writeErrFrame(ERR_CODE_BAD_STATE);
               ...
               len - 10 < MAX_PACKET_PAYLOAD - 5) {
        uint8_t path_len = len - 10;
        uint8_t flags = cmd_frame[9];
        uint8_t path_sz = flags & 0x03; // NEW v1.11+
        if ((path_len >> path_sz) > MAX_PATH_SIZE ||
            (path_len % (1 << path_sz)) !=
                0) { // make sure is multiple of path_sz
            writeErrFrame(ERR_CODE_ILLEGAL_ARG);
        } else {
            uint32_t tag, auth;
            ...
            if (pkt) {
                ...
                uint32_t t = _radio->getEstAirtimeFor(pkt->payload_len +
                                                      pkt->path_len + 2);
                uint32_t est_timeout =
                    calcDirectTimeoutMillisFor(t, path_len >> path_sz);
                out_frame[0] = RESP_CODE_SENT;
                out_frame[1] = 0;
                memcpy(&out_frame[2], &tag, 4);
                memcpy(&out_frame[6], &est_timeout, 4);
        ...
        char *const end = dp + 140;
        for (int i = 0; i < sensors.getNumSettings() && dp < end; i++) {
            if (i > 0) {
                if (dp + 1 >= end)
                    break;
                *dp++ = ',';
            ...
            StrHelper::strncpy(dp, name ? name : "", end - dp);
            dp += strlen(dp);
            if (dp + 1 >= end)
                break;
            *dp++ = ':';
            const char *value = sensors.getSettingValue(i);
            StrHelper::strncpy(dp, value ? value : "", end - dp);
            dp += strlen(dp);
        }
        ...
        for (int i = 0; i < ADVERT_PATH_TABLE_SIZE; i++) {
            ...
            if (memcmp(p->pubkey_prefix, pub_key, sizeof(p->pubkey_prefix)) ==
                0) {
                found = p;
                break;
            }
        ...
        if (found) {
            ...
            i += mesh::Packet::writePath(&out_frame[i], found->path,
                                         found->path_len);
            _serial->writeFrame(out_frame, i);
        } else {
            writeErrFrame(ERR_CODE_NOT_FOUND);
        ...
        if (stats_type == STATS_TYPE_CORE) {
            ...
            int8_t last_snr = (int8_t)(radio_driver.getLastSNR() *
                                       4); // scaled by 4 for 0.25 dB precision
    ...
    } else if (cmd_frame[0] == CMD_FACTORY_RESET &&
               memcmp(&cmd_frame[1], "reset", 5) == 0) {
        if (_serial) {
            MESH_DEBUG_PRINTLN("Factory reset: disabling serial interface to "
                               "prevent reconnects (BLE/WiFi)");
            _serial->disable(); // Phone app disconnects before we can send OK
                                // frame so it's safe here
        }
    ...
    } else if (cmd_frame[0] == CMD_SET_FLOOD_SCOPE_KEY && len >= 2 &&
               cmd_frame[1] == 0) {
        if (len >= 2 + 16) {
            memcpy(send_scope.key, &cmd_frame[2],
                   sizeof(send_scope.key)); // set scope override TransportKey
        } else {
            memset(send_scope.key, 0,
                   sizeof(send_scope.key)); // reset scope override
        }
        send_unscoped = false;
        writeOKFrame();
    } else if (cmd_frame[0] == CMD_SET_FLOOD_SCOPE_KEY && len >= 2 &&
               cmd_frame[1] == 1) { // ver 12+
        send_unscoped = true;
        writeOKFrame();
        ...
        if (len >= 1 + 31 + 16) {
            ...
            memset(
                _prefs.default_scope_name, 0,
                sizeof(_prefs.default_scope_name)); // set default scope to null
            memset(_prefs.default_scope_key, 0,
    ...
    } else if (cmd_frame[0] == CMD_SEND_CONTROL_DATA && len >= 2 &&
               (cmd_frame[1] & 0x80) != 0) {
        ...
        for (size_t k = 0;
             k < sizeof(repeat_freq_ranges) / sizeof(repeat_freq_ranges[0]) &&
             static_cast<size_t>(i + 8) < sizeof(out_frame);
             k++) {
...
/**
 * @brief Saves contacts through the configured DataStore.
 * @param None.
 * @return None.
 */
void MyMesh::saveContacts() { _store->saveContacts(this, save_filter); }
...
void MyMesh::checkCLIRescueCmd() {
    ...
    if (len > 0 && cli_command[len - 1] == '\r') { // received complete line
        cli_command[len - 1] =
            0; // replace newline with C string null terminator
        if (memcmp(cli_command, "set ", 4) == 0) {
            ...
            if (is_fs2 == true || strlen(path) == 0 || strcmp(path, "/") == 0) {
                if (_store->getSecondaryFS() != nullptr) {
                    File root2 =
                        _store->openRead(_store->getSecondaryFS(), path);
            ...
            if (memcmp(path, "UserData/", 9) == 0) {
                ...
                Serial.println("Invalid path provided, must start with "
                               "UserData/ or ExtraFS/");
                cli_command[0] = 0;
                return;
            }
            ...
            if (file) {
                ...
                // read/print in fixed-size chunks: avoids a VLA (non-standard
                // C++) and an unbounded stack allocation for large files
                uint8_t buffer[64];
                int remaining = file_size;
                while (remaining > 0) {
                    int chunk = remaining < (int)sizeof(buffer)
                                    ? remaining
                                    : (int)sizeof(buffer);
                    file.read(buffer, chunk);
                    mesh::Utils::printHex(Serial, buffer, chunk);
                    remaining -= chunk;
                }
                Serial.print("\n");
                file.close();
...
void MyMesh::checkSerialInterface() {
    ...
    if (len > 0) {
        ...
        if (_iter.hasNext(this, contact)) {
            if (contact.lastmod >
                _iter_filter_since) { // apply the 'since' filter
                writeContactRespFrame(RESP_CODE_CONTACT, contact);
                if (contact.lastmod > _most_recent_lastmod) {
                    _most_recent_lastmod =
                        contact.lastmod; // save for the
                                         // RESP_CODE_END_OF_CONTACTS frame
                }
            }
        } else { // EOF
            out_frame[0] = RESP_CODE_END_OF_CONTACTS;
            memcpy(&out_frame[1], &_most_recent_lastmod,
                   4); // include the most recent lastmod, so app can update
                       // their 'since'
...
/**
 * @brief Reports whether the node has pending application work.
 * @param None.
