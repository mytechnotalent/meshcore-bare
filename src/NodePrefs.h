/** @file NodePrefs.h
 *  @brief Persisted node configuration.
 */
...
/** Stores the node settings serialized by DataStore. */
class NodePrefs : public ConfigSerializer { // persisted to file
  public:
    ...
    uint8_t rx_boosted_gain =
        0; // SX126x RX boosted gain mode (0=power saving, 1=boosted)
    uint8_t _client_repeat = 0; // DEPRECATED -> use repeat.disable_fwd
    uint8_t path_hash_mode =
        1; // which path mode to use when sending (1 = 2-byte)
    uint8_t autoadd_max_hops =
        0; // 0 = no limit, 1 = direct (0 hops), N = up to N-1 hops (max 64)
    char default_scope_name[31];
    uint8_t default_scope_key[16];

  private:
    class RadioPrefs
        : public ConfigSerializer { // COPIED from CommonCLI (for now)
        NodePrefs *_parent;

      protected:
        /**
         * @brief Registers radio preferences for serialization.
         * @param None.
         * @return None.
         */
        void structure() override {
            ...
            // def("f_txdelay", _parent->tx_delay_factor);   currently
            // hard-coded def("d_txdelay", _parent->direct_tx_delay_factor);
            // currently hard-coded def("agc_int", _parent->agc_reset_interval);
      ...
      public:
        /**
         * @brief Creates a radio preference view.
         * @param parent Owning node preferences.
         * @return None.
         */
    ...
    class GPSPrefs
      ...
      protected:
        /**
         * @brief Registers GPS preferences for serialization.
         * @param None.
         * @return None.
         */
        void structure() override {
      ...
      public:
        /**
         * @brief Creates a GPS preference view.
         * @param parent Owning node preferences.
         * @return None.
         */
    ...
    class RepeatPrefs
      ...
      protected:
        /**
         * @brief Registers repeat preferences for serialization.
         * @param None.
         * @return None.
         */
        void structure() override {
    ...
    class CompanionPrefs : public ConfigSerializer {
      ...
      protected:
        /**
         * @brief Registers companion preferences for serialization.
         * @param None.
         * @return None.
         */
        void structure() override {
            def("auto_max",
                _parent
                    ->autoadd_max_hops); // 0 = no limit, 1 = direct (0 hops), N
                                         // = up to N-1 hops (max 64)
            def("defs_nm", _parent->default_scope_name,
                sizeof(_parent->default_scope_name));
            def("defs_key", (void *)_parent->default_scope_key,
                sizeof(_parent->default_scope_key));
            def("pin", _parent->ble_pin);
            def("buzz_q", _parent->buzzer_quiet);
            def("vibe_q", _parent->vibe_quiet);
            def("auto_add",
                _parent
                    ->autoadd_config); // bitmask for auto-add contacts config
      ...
      public:
        /**
         * @brief Creates a companion preference view.
         * @param parent Owning node preferences.
         * @return None.
         */
  ...
  protected:
    /**
     * @brief Registers all node preferences for serialization.
     * @param None.
     * @return None.
     */
    void structure() override {
  ...
  public:
    /**
     * @brief Creates preferences with safe empty defaults.
     * @param None.
     * @return None.
     */
    ...
    /**
     * @brief Returns whether packet forwarding is enabled.
     * @param None.
     * @return true when forwarding is enabled; otherwise false.
     */
    bool isRepeatEn() const { return repeat.disable_fwd == 0; }
    /**
     * @brief Enables or disables packet forwarding.
     * @param en Whether forwarding should be enabled.
     * @return None.
     */
